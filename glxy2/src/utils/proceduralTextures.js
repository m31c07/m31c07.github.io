...
export class ProceduralPlanetTexture {
  constructor(starX, starY, planetIndex, planetType, textureSize = 512, moonIndex = 0) {
    // Create deterministic seed from star position, planet index, and optional moon index
  ...
  // UV Elevation map generation for wider texture (fractal + warp)
  generateUVElevationMap(uvWidth, uvHeight) {
    const elevation = new Float32Array(uvWidth * uvHeight);
    const elevOct = this.typeParams.noiseOctavesElev || 6;
    const lacE = this.typeParams.lacunarityElev || 2.0;
    const gainE = this.typeParams.gainElev || 0.5;
    const ridgeW = this.typeParams.ridgeWeight ?? 0.5;
    const warpStrength = this.typeParams.warpStrength ?? 0.35;
    const warpOct = this.typeParams.warpOctaves || 3;

    let sumAmp = 0, amp = 1.0;
    for (let i = 0; i < elevOct; i++) { sumAmp += amp; amp *= gainE; }
    ...
    for (let y = 0; y < uvHeight; y++) {
      for (let x = 0; x < uvWidth; x++) {
        ...
        const lon = (normalizedX / baseWidth) * 2 * Math.PI;
        ...
        const freqBase = 2.0 * this.typeParams.continentScale;
        const [wx, wy, wz] = this.domainWarp3D(sx * freqBase, sy * freqBase, sz * freqBase, warpStrength, warpOct, lacE, gainE);

        const ridge = this.ridgedFbm3D(wx, wy, wz, elevOct, lacE, gainE);
        const fbm = this.fbm3D(wx + 127.4, wy + 127.4, wz + 127.4, elevOct, lacE, gainE);

        const fbmNorm = (fbm / sumAmp + 1) * 0.5;
        const ridgeNorm = ridge / sumAmp;

        let value = (1 - ridgeW) * fbmNorm + ridgeW * ridgeNorm;

        if (this.planetType === 'terran' || this.planetType === 'rocky') {
          value = this.applyContinentFormation(value, lat, lon);
        }

        elevation[index] = Math.max(-1, Math.min(1, value * 2 - 1));
      }
    }

    const erosionIters = this.typeParams.erosionIterations || 0;
    if (erosionIters > 0) {
      this.applyErosionUV(elevation, uvWidth, uvHeight, erosionIters);
    }
  ...
  // UV Humidity map generation (fractal + circulation + elevation influence)
  generateUVHumidityMap(uvWidth, uvHeight, elevationMap) {
    const humidity = new Float32Array(uvWidth * uvHeight);
    const humOct = this.typeParams.noiseOctavesHumid || 4;
    const lacH = this.typeParams.lacunarityHumid || 2.0;
    const gainH = this.typeParams.gainHumid || 0.55;
    const cells = this.typeParams.circulationCells || 3;
    const warpStrength = (this.typeParams.warpStrength || 0.35) * 0.5;
    const warpOct = this.typeParams.warpOctaves || 3;

    let sumAmp = 0, amp = 1.0;
    for (let i = 0; i < humOct; i++) { sumAmp += amp; amp *= gainH; }
    ...
    for (let y = 0; y < uvHeight; y++) {
      for (let x = 0; x < uvWidth; x++) {
        ...
        const [wx, wy, wz] = this.domainWarp3D(sx, sy, sz, warpStrength, warpOct, lacH, gainH);

        const humNoise = this.fbm3D(wx * 1.5 + 100, wy * 1.5 + 100, wz * 1.5 + 100, humOct, lacH, gainH);
        let value = (humNoise / sumAmp + 1) * 0.5;

        const equatorEffect = Math.cos(lat - Math.PI / 2);
        const hadley = Math.sin(cells * (lat - Math.PI / 2)) * 0.5 + 0.5;

        value = value * (0.4 + 0.6 * hadley) * this.typeParams.humidityVariation + 0.2;
        value *= Math.max(0.2, 0.2 + 0.8 * equatorEffect);

        if (elevationMap) {
          const elev = elevationMap[index];
          if (elev > 0) {
            value *= (1 - Math.min(0.35, elev * 0.35));
          }
        }
  ...
  generateUVTexture(rotationWidth = 1.5) {
    ...
    const humidityMap = this.generateUVHumidityMap(uvWidth, uvHeight, elevationMap);
    ...
    // Apply type-specific detail overlays to enhance realism in UV unwrap
    this.applyTypeSpecificUVDetails(textureData, elevationMap, humidityMap, uvWidth, uvHeight, baseWidth);
  ...
  // Add fine-grained, type-specific details to UV texture data while respecting seams
  applyTypeSpecificUVDetails(textureData, elevationMap, humidityMap, uvWidth, uvHeight, baseWidth) {
    switch (this.planetType) {
      case 'gas':
        return this.applyGasBandsUV(textureData, uvWidth, uvHeight);
      case 'desert':
        return this.applyDesertDunesUV(textureData, elevationMap, uvWidth, uvHeight, baseWidth);
      case 'volcanic':
      case 'lava':
        return this.applyVolcanicLavaUV(textureData, elevationMap, uvWidth, uvHeight);
      case 'ice':
        return this.applyIceCracksUV(textureData, elevationMap, uvWidth, uvHeight);
      default:
        return; // other types already look good with biome coloring
    }
  }

  // Gas giants: latitudinal bands with noisy variation and subtle storms
  applyGasBandsUV(textureData, uvWidth, uvHeight) {
    const bandCount = 10 + Math.floor(this.rng() * 6); // 10-16 main bands
    const bandContrast = 0.10; // intensity of band brightness variation
    for (let y = 0; y < uvHeight; y++) {
      const lat = (y / uvHeight) * Math.PI; // 0..PI
      const equatorFactor = Math.sin(lat); // max near equator
      const bandPhase = bandCount * lat + this.noise.noise3D(0, y * 0.02, this.seed * 0.001) * 1.5;
      const bandValue = Math.sin(bandPhase) * bandContrast * (0.7 + 0.3 * equatorFactor);
      for (let x = 0; x < uvWidth; x++) {
        const idx = (y * uvWidth + x) * 4;
        textureData[idx]     = Math.max(0, Math.min(255, textureData[idx]     * (1 + bandValue)));
        textureData[idx + 1] = Math.max(0, Math.min(255, textureData[idx + 1] * (1 + bandValue)));
        textureData[idx + 2] = Math.max(0, Math.min(255, textureData[idx + 2] * (1 + bandValue)));
      }
    }
    // add a few storm ovals
    const storms = 3 + Math.floor(this.rng() * 4);
    for (let s = 0; s < storms; s++) {
      const cx = Math.floor(this.rng() * (uvWidth - 2)) + 1;
      const cy = Math.floor(this.rng() * uvHeight);
      const rx = 8 + Math.floor(this.rng() * 20);
      const ry = 6 + Math.floor(this.rng() * 16);
      const intensity = 0.18 + this.rng() * 0.12;
      for (let dy = -ry; dy <= ry; dy++) {
        const y = cy + dy;
        if (y < 0 || y >= uvHeight) continue;
        for (let dx = -rx; dx <= rx; dx++) {
          let x = cx + dx;
          // wrap horizontally (seam-safe)
          if (x < 0) x += (uvWidth - 2);
          if (x >= uvWidth - 2) x -= (uvWidth - 2);
          const nx = dx / rx;
          const ny = dy / ry;
          const d = nx * nx + ny * ny;
          if (d <= 1) {
            const falloff = 1 - d;
            const idx = (y * uvWidth + x) * 4;
            textureData[idx]     = Math.max(0, Math.min(255, textureData[idx]     * (1 + intensity * falloff)));
            textureData[idx + 1] = Math.max(0, Math.min(255, textureData[idx + 1] * (1 + intensity * falloff)));
            textureData[idx + 2] = Math.max(0, Math.min(255, textureData[idx + 2] * (1 + intensity * falloff)));
          }
        }
      }
    }
  ...
  // Desert: anisotropic dune stripes aligned by seeded wind direction
  applyDesertDunesUV(textureData, elevationMap, uvWidth, uvHeight, baseWidth) {
    const theta = this.rng() * Math.PI * 2; // wind direction
    const cosT = Math.cos(theta);
    const sinT = Math.sin(theta);
    const freq = 0.06 + this.rng() * 0.04; // dune frequency
    const amp = 0.10; // brightness modulation amplitude
    for (let y = 0; y < uvHeight; y++) {
      for (let x = 0; x < uvWidth; x++) {
        // wrap base width for seam safety when computing coordinate t
        let normalizedX = x;
        if (x >= baseWidth) normalizedX = x - baseWidth;
        const t = normalizedX * cosT + y * sinT;
        const noiseJitter = this.noise.noise3D(normalizedX * 0.02, y * 0.02, this.seed * 0.001);
        const stripe = Math.sin(t * freq + noiseJitter * 2.0);
        const idx = (y * uvWidth + x) * 4;
        // only modulate bright sandy areas: detect via average channel value
        const avg = (textureData[idx] + textureData[idx + 1] + textureData[idx + 2]) / 3;
        if (avg > 140) {
          const m = 1 + amp * stripe;
          textureData[idx]     = Math.max(0, Math.min(255, textureData[idx]     * m));
          textureData[idx + 1] = Math.max(0, Math.min(255, textureData[idx + 1] * m));
          textureData[idx + 2] = Math.max(0, Math.min(255, textureData[idx + 2] * m));
        }
      }
    }
  }

  // Volcanic/lava: bright lava veins using ridged noise; blended additively
  applyVolcanicLavaUV(textureData, elevationMap, uvWidth, uvHeight) {
    const threshold = 0.6; // ridge threshold
    const glow = 40; // additive glow value
    for (let y = 0; y < uvHeight; y++) {
      const lat = (y / uvHeight) * Math.PI;
      for (let x = 0; x < uvWidth; x++) {
        const lon = (x / (uvWidth - 2)) * 2 * Math.PI;
        const sx = Math.sin(lat) * Math.cos(lon);
        const sy = Math.sin(lat) * Math.sin(lon);
        const sz = Math.cos(lat);
        // ridged noise
        const n = Math.abs(this.noise.noise3D(sx * 3.0, sy * 3.0, sz * 3.0));
        if (n > threshold) {
          const idx = (y * uvWidth + x) * 4;
          // lava color blend
          textureData[idx]     = Math.min(255, textureData[idx]     + glow + 60);
          textureData[idx + 1] = Math.min(255, textureData[idx + 1] + glow);
          textureData[idx + 2] = Math.max(0,   textureData[idx + 2] - 20);
        }
      }
    }
  }

  // Ice: subtle crack lines (dark) stronger near poles
  applyIceCracksUV(textureData, elevationMap, uvWidth, uvHeight) {
    for (let y = 0; y < uvHeight; y++) {
      const lat = (y / uvHeight) * Math.PI;
      const poleFactor = Math.min(lat, Math.PI - lat) / (Math.PI / 2); // 0 at pole, 1 at equator
      const crackAmp = (1 - poleFactor) * 0.25; // stronger near poles
      for (let x = 0; x < uvWidth; x++) {
        const lon = (x / (uvWidth - 2)) * 2 * Math.PI;
        const sx = Math.sin(lat) * Math.cos(lon);
        const sy = Math.sin(lat) * Math.sin(lon);
        const sz = Math.cos(lat);
        const ridged = Math.abs(this.noise.noise3D(sx * 6.0, sy * 6.0, sz * 6.0));
        if (ridged > 0.7) {
          const idx = (y * uvWidth + x) * 4;
          const m = 1 - crackAmp * (ridged - 0.7) * 3;
          textureData[idx]     = Math.max(0, Math.min(255, textureData[idx]     * m));
          textureData[idx + 1] = Math.max(0, Math.min(255, textureData[idx + 1] * m));
          textureData[idx + 2] = Math.max(0, Math.min(255, textureData[idx + 2] * m));
        }
  ...
  // Fractal noise helpers for realism
  ...
  ridgedFbm3D(x, y, z, octaves = 6, lacunarity = 2.0, gain = 0.5) {
    let sum = 0, amp = 1.0, freq = 1.0;
    for (let i = 0; i < octaves; i++) {
      let n = this.noise.noise3D(x * freq, y * freq, z * freq);
      n = 1.0 - Math.abs(n);
      n *= n;
      sum += n * amp;
      freq *= lacunarity;
  ...
  domainWarp3D(x, y, z, strength = 0.35, octaves = 3, lacunarity = 2.0, gain = 0.5) {
    const wx = this.fbm3D(x + 13.37, y + 47.11, z + 23.89, octaves, lacunarity, gain);
    const wy = this.fbm3D(x - 29.41, y + 5.03, z - 11.79, octaves, lacunarity, gain);
    const wz = this.fbm3D(x + 7.07, y - 17.31, z + 31.01, octaves, lacunarity, gain);
    return [x + strength * wx, y + strength * wy, z + strength * wz];
  }

  // Seam-aware gentle erosion/smoothing
  applyErosionUV(elevation, uvWidth, uvHeight, iterations = 1) {
    for (let it = 0; it < iterations; it++) {
      const src = elevation.slice();
      for (let y = 0; y < uvHeight; y++) {
        for (let x = 0; x < uvWidth; x++) {
          const idx = y * uvWidth + x;
          const leftIdx = y * uvWidth + ((x - 1 + uvWidth) % uvWidth);
          const rightIdx = y * uvWidth + ((x + 1) % uvWidth);
          const upIdx = (y > 0 ? y - 1 : y) * uvWidth + x;
          const downIdx = (y < uvHeight - 1 ? y + 1 : y) * uvWidth + x;
          elevation[idx] =
            src[idx] * 0.4 +
            src[leftIdx] * 0.15 +
            src[rightIdx] * 0.15 +
...
// Унифицированный хелпер для генерации ключа/нормализации сид-параметров
export function makeSeedKey(star, planetIndex, moonIndex = 0) {
  const starX = (star && typeof star.x === 'number') ? star.x : ((star && typeof star.id === 'number') ? star.id * 1000 : 0);
  const starY = (star && typeof star.y === 'number') ? star.y : ((star && typeof star.id === 'number') ? star.id * 1000 : 0);
  const nx = Math.floor(starX * 1000) % 10000;
  const ny = Math.floor(starY * 1000) % 10000;
  const mi = (moonIndex || 0) % 1000;
  return { starX, starY, planetIndex, moonIndex: mi, key: `${nx}_${ny}_${planetIndex}_${mi}` };
}
export function generatePlanetTexture(starX, starY, planetIndex, planetType, size = 512, moonIndex = 0) {
  const cacheKey = `${Math.floor(starX * 1000)}_${Math.floor(starY * 1000)}_${planetIndex}_${moonIndex}_${planetType}_${size}`;
  
  if (textureCache.has(cacheKey)) {
    return textureCache.get(cacheKey);
  }
  
  const generator = new ProceduralPlanetTexture(starX, starY, planetIndex, planetType, size, moonIndex);
...
// Helper function to create UV planet texture for rotation animation
export function generatePlanetUVTexture(
  starX,
  starY,
  planetIndex,
  planetType,
  ...
  const cacheKey = `uv_${Math.floor(starX * 1000)}_${Math.floor(starY * 1000)}_${planetIndex}_${moonIndex}_${planetType}_${size}_${rotationWidth}`;
  ...
  // === подготовка данных для оптимизированного рисования ===
  const slices = 80; // баланс между качеством и скоростью

  // удвоенная текстура для wrap-around
  const textureCanvas = document.createElement('canvas');
  textureCanvas.width = uvTexture.width * 2;
  textureCanvas.height = uvTexture.height;
  const textureCtx = textureCanvas.getContext('2d', { willReadFrequently: true });
  textureCtx.drawImage(uvTexture, 0, 0);
  textureCtx.drawImage(uvTexture, uvTexture.width, 0);

  const sliceHeightDest = (size * 2) / slices;
  const sliceHeightTex = uvTexture.height / slices;
  const factors = [];

  for (let i = 0; i < slices; i++) {
    const relativeY = (i + 0.5 - slices / 2) / (slices / 2);
    const absY = Math.abs(relativeY);

    // выпуклая форма: центр широкий, края узкие
    const stretchFactor = Math.sqrt(1 - absY * absY) * (1.0 + 0.3 * (1 - absY));

    factors.push({
      yDest: i * sliceHeightDest,
      sliceHeightDest,
      yTex: i * sliceHeightTex,
      sliceHeightTex,
      stretchFactor,
    });
  }

  // добавляем метаданные в саму текстуру
  uvTexture._sliceData = { textureCanvas, factors, planetSize: size };

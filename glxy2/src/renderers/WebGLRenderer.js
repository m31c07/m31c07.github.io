...
export default class WebGLRenderer {
  ...
  initPlanet2DShader() {
    const vsSource = `
      ...
      void main() {
        ...
        vec2 lightDir = normalize(uPlanetPosition - uStarPosition);
    ...
    const fsSource = `
      ...
      void main() {
        ...
        if (uUseTexture) {
          // Calculate spherical coordinates for 3D appearance
          float r = sqrt(r2);
          float z = sqrt(1.0 - r2); // z-coordinate on sphere
          
          // Convert to spherical texture coordinates with sinusoidal distortion
          float longitude = atan(p.x, z) / (2.0 * 3.14159265) + 0.5;
          float latitude = asin(p.y / sqrt(p.x * p.x + p.y * p.y + z * z)) / 3.14159265 + 0.5;
          
          // Apply rotation offset
          longitude += uRotationOffset;
          longitude = mod(longitude, 1.0);
          
          // Sample texture with spherical coordinates
          vec2 sphericalTexCoord = vec2(longitude, latitude);
          vec3 textureColor = texture2D(uTexture, sphericalTexCoord).rgb;
          
          // Calculate lighting based on star position
          // Surface normal at this point on the sphere
          vec3 surfaceNormal = normalize(vec3(p.x, p.y, z));
          
          // Light direction from star (convert 2D light direction to 3D and invert)
          vec3 lightDir3D = normalize(vec3(-vLightDirection.x, -vLightDirection.y, 0.3));
          
          // Calculate diffuse lighting with proper sphere geometry
          float lightAngle = dot(surfaceNormal, lightDir3D);
          float shading = 0.3 + 0.7 * max(0.0, lightAngle);
          
          // Add rim lighting for atmospheric effect
          float rimLight = 1.0 - r;
          rimLight = pow(rimLight, 3.0) * 0.15;
          
          finalColor = textureColor * shading + textureColor * rimLight;
        } else {
          // Fallback to solid color with enhanced spherical shading
          float r = sqrt(r2);
          float z = sqrt(1.0 - r2);
          ...
          finalColor = uColor.rgb * shading + uColor.rgb * rimLight;
        }
  ...
  // Initialize blurred background planet shader (for moon view backgrounds)
  initBlurredPlanetShader() {
    const vsSource = `
      attribute vec2 aPosition;
      uniform mat4 uProjectionMatrix;
      uniform mat4 uViewMatrix;
      ...
      void main() {
        ...
        // Calculate light direction from planet to star (corrected direction)
        vLightDirection = normalize(uStarPosition - uPlanetPosition);
      }
    `;
    ...
    const fsSource = `
      ...
      void main() {
        ...
        if (uUseTexture) {
          ...
          // Convert to spherical texture coordinates
          float longitude = atan(p.x, z) / (2.0 * 3.14159265) + 0.5;
          float latitude = asin(p.y / sqrt(p.x * p.x + p.y * p.y + z * z)) / 3.14159265 + 0.5;
          ...
          // Sample multiple points for blur effect with spherical coordinates
          vec3 textureColor = vec3(0.0);
          float blurStep = uBlurAmount * 0.001; // Reduced for spherical coordinates
          int samples = 9;
          
          for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 1; j++) {
              vec2 offset = vec2(float(i), float(j)) * blurStep;
              vec2 sampleCoord = vec2(longitude, latitude) + offset;
              sampleCoord.x = mod(sampleCoord.x, 1.0);
              sampleCoord.y = clamp(sampleCoord.y, 0.0, 1.0);
              textureColor += texture2D(uTexture, sampleCoord).rgb;
            }
          }
          textureColor /= float(samples);
          
          // Enhanced atmospheric lighting for spherical surface using star position
          vec3 lightDir3D = normalize(vec3(-vLightDirection.x, -vLightDirection.y, 0.5));
          float lightAngle = dot(normalize(vec3(p.x, p.y, z)), lightDir3D);
          float shading = 0.3 + 0.5 * max(0.0, lightAngle);
          
          // Strong atmospheric glow for background effect
          float atmosphereGlow = 1.0 - r2 * 0.2;
          atmosphereGlow = pow(atmosphereGlow, 0.8);
          
          // Rim lighting for atmospheric halo
          float rimLight = 1.0 - r;
          rimLight = pow(rimLight, 2.5) * 0.2;
          
          finalColor = textureColor * shading * atmosphereGlow + textureColor * rimLight;
          
          // Add subtle color tint for atmospheric effect
          finalColor = mix(finalColor, uColor.rgb, 0.15);
        } else {
          // Fallback to soft colored sphere with spherical shading using star position
          float r = sqrt(r2);
          float z = sqrt(1.0 - r2);
          vec3 lightDir3D = normalize(vec3(-vLightDirection.x, -vLightDirection.y, 0.5));
          float lightAngle = dot(normalize(vec3(p.x, p.y, z)), lightDir3D);
          ...
          float rimLight = 1.0 - r;
          rimLight = pow(rimLight, 2.5) * 0.15;
          
          finalColor = uColor.rgb * shading * atmosphereGlow + uColor.rgb * rimLight;
        }
        
        // Reduce opacity for background effect with atmospheric falloff
        float alpha = uColor.a * (1.0 - r2 * 0.1);
        gl_FragColor = vec4(finalColor, alpha);
      }
    `;
    
    this.programs.blurredPlanet = this.createShaderProgram(vsSource, fsSource);
  }
  ...
  initBasicShaders() {
    ...
    // Blurred planet shader (for background planets in moon view)
    this.initBlurredPlanetShader();
  }
    ...
    try {
      ...
      switch (object.type) {
        ...
        case 'moon2D':
          ...
          break;
        case 'blurredPlanet':
          program = this.programs.blurredPlanet;
          break;
        default:
          program = this.programs.point;
      }
      ...
      // Handle 2D planet, moon, and blurred planet objects
      else if (object.type === 'planet2D' || object.type === 'moon2D' || object.type === 'blurredPlanet') {
        ...
        // Set blur amount for blurred planets
        if (object.type === 'blurredPlanet') {
          const blurLoc = this.gl.getUniformLocation(program, 'uBlurAmount');
          if (blurLoc) {
            const blurAmount = object.blurAmount || 5.0;
            this.gl.uniform1f(blurLoc, blurAmount);
          }
        }
        
        // Set star and planet positions for lighting calculation
        if (object.type === 'planet2D' || object.type === 'moon2D' || object.type === 'blurredPlanet') {
  ...
  // Render a blurred background planet (for moon view backgrounds)
  renderBlurredPlanet(x, y, size, color, texture = null, rotationOffset = 0.0, blurAmount = 5.0) {
    const program = this.programs.blurredPlanet;
    if (!program) return;

    this.gl.useProgram(program);

    // Set up vertex buffer for a single point
    ...
    if (projectionLoc) this.gl.uniformMatrix4fv(projectionLoc, false, this.projectionMatrix);

    const viewLoc = this.gl.getUniformLocation(program, 'uViewMatrix');
    if (viewLoc) this.gl.uniformMatrix4fv(viewLoc, false, this.viewMatrix);

    const sizeLoc = this.gl.getUniformLocation(program, 'uPointSize');
    if (sizeLoc) this.gl.uniform1f(sizeLoc, size);
    ...
    if (colorLoc) {
      ...
      if (colorArray.length !== 4) {
        colorArray = new Float32Array([colorArray[0] || 1.0, colorArray[1] || 1.0, colorArray[2] || 1.0, colorArray[3] || 0.7]);
      }
      this.gl.uniform4fv(colorLoc, colorArray);
    }

    const rotationLoc = this.gl.getUniformLocation(program, 'uRotationOffset');
    if (rotationLoc) this.gl.uniform1f(rotationLoc, rotationOffset);

    const blurLoc = this.gl.getUniformLocation(program, 'uBlurAmount');
    if (blurLoc) this.gl.uniform1f(blurLoc, blurAmount);

    // Handle texture
    const useTextureLoc = this.gl.getUniformLocation(program, 'uUseTexture');
    if (texture) {
      if (useTextureLoc) this.gl.uniform1i(useTextureLoc, 1);
      
      const textureLoc = this.gl.getUniformLocation(program, 'uTexture');
      if (textureLoc) {
        this.gl.activeTexture(this.gl.TEXTURE0);
        this.gl.bindTexture(this.gl.TEXTURE_2D, this.createTextureFromCanvas(texture));
        this.gl.uniform1i(textureLoc, 0);
      }
    } else {
      if (useTextureLoc) this.gl.uniform1i(useTextureLoc, 0);
    }
